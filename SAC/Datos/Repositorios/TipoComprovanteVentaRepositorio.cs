   ...
   public class TipoComprovanteVentaRepositorio : RepositorioBase<TipoComprobante>
    {
        ...
        {
            context.Configuration.LazyLoadingEnabled = false;
            return context.TipoComprobanteVenta.Where(p => p.Id == id).Select(p => p.Numero).FirstOrDefault();
       ...
       public TipoComprobanteVenta getTipoComprobanteVentaNewNumeroFactura(int nroComprobante, int puntoVenta)
        {
            context.Configuration.LazyLoadingEnabled = false;
            //var nroFactura = context.TipoComprobanteVenta.Where(p => p.CodigoAfip == nroComprobante
            //                                                    && p.PuntoVenta == puntoVenta).First();
